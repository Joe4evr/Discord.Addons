...
public sealed class AudioService
{
    ...
    private async Task Client_ReactionAdded(Cacheable<IUserMessage, ulong> message, ISocketMessageChannel channel, SocketReaction reaction)
    {
        ...
        if (msg.Channel is not IGuildChannel gch)
        {
            return;
        }

        // Please tell me this restriction on patterns
        // will lift in some future version... ><
        if (GetWrapper(gch.Guild) is { /*Message.Id: msg.Id*/ } wrapper
